...
The api.cache.cache module is patched rather than driven through a real Redis,
so these assert the caching *logic*: a miss falls through to the database and
...
class TestGetUserProfileCacheWrite:
    ...
    @patch("api.user.user_services.cache")
    ...
    def test_a_miss_stores_the_response_with_a_ttl(
        ...
        key, payload = mock_cache.set.call_args.args

        assert key.startswith("user:")
        assert mock_cache.set.call_args.kwargs["ttl"] == CACHE_TTL_SECONDS
    ...
    def test_the_cache_is_read_before_the_database(
        ...
        read_key = mock_cache.get.call_args.args[0]
...
class TestGetUserProfileCacheRead:
    ...
    def test_a_hit_is_served_without_touching_the_database(
        ...
        mock_cache.get.return_value = cached.model_dump_json()
    ...
    def test_unreadable_cached_json_falls_back_to_the_database(
        ...
        """
        mock_cache.get.return_value = "not-json-at-all"
        mock_resolve.return_value = (make_user_row(), True)
...
class TestGetUserProfileCacheKeyIsolation:
    ...
    def test_owner_and_stranger_do_not_share_an_entry(
        ...
        owner_key = mock_cache.get.call_args_list[0].args[0]
        stranger_key = mock_cache.get.call_args_list[1].args[0]
    ...
    def test_different_profiles_do_not_share_an_entry(
        ...
        first_key = mock_cache.get.call_args_list[0].args[0]
        second_key = mock_cache.get.call_args_list[1].args[0]
